...
void __attribute__((naked)) task4(void)
{
    ...
    while (1) {
        ...
        if (cmd_index >= 4 && cmd_buffer[0] == 'e' && cmd_buffer[1] == 'c' &&
            ...
            if (target_addr != EXTERN_NULL) {
                ...
                for (uint16_t i = 0; i < 512; i += 64) {
                    extern_memory_read(target_buf + i, (char *) picos_cache);
                    for (uint16_t j = 0; j < 64; j++) {
                        if (i + j == target->file_size)
                            goto end_cat_release;
            ...
            GIE = 1;
        } else if (cmd_index >= 1 && cmd_buffer[0] == 'w') {
            uint8_t idx = 1;

            while (idx < cmd_index && cmd_buffer[idx] == ' ')
                idx++;
            if (idx == cmd_index) {
                str = "Usage: w <file> <content>\r\n";
                for (uint8_t i = 0; i < 26; i++)
                    usr_uart_put_char(str[i]);
                continue;
            }

            uint8_t fname_start = idx;
            while (idx < cmd_index && cmd_buffer[idx] != ' ')
                idx++;
            uint8_t fname_len = idx - fname_start;
            ...
            uint8_t content_start = idx;
            uint8_t content_len = cmd_index - content_start;

            GIE = 0;
            addr_t target_addr =
                find_file(root, &cmd_buffer[fname_start], fname_len);
            if (target_addr != EXTERN_NULL) {
                extern_memory_read(target_addr, (char *) file_cache);
                file_t *target = (file_t *) file_cache;

                write_file(fs, target, &cmd_buffer[content_start], content_len);
                extern_memory_write(target_addr, (char *) file_cache);
